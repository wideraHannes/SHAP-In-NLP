...
def calculate_thresholds(values, low_percentile, high_percentile):
    low_threshold = np.percentile(values, low_percentile)
    high_threshold = np.percentile(values, high_percentile)
    return low_threshold, high_threshold

def find_lowest_highest_words_indices(words, values, low_threshold, high_threshold):
...
def highlight_word(word, value, background_color):
...
def highlight_signal_words(shap_values,round_shap_values=3, top_words=5):
    ...
    low_threshold, high_threshold = calculate_thresholds(values, low_percentile, high_percentile)
    lowest_word_index_value, highest_word_index_value = find_lowest_highest_words_indices(words, values, low_threshold, high_threshold)
